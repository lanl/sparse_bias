...
parameters {
  ...
  array[n_levels-1] vector[nb_m]          gamma_tilde_m;
  array[n_levels-1] vector<lower=0>[nb_m]      lambda_m;
  real<lower=0>                                   tau_m;
}
transformed parameters {
  ...
  for (i in 1:(n_levels-1)){
    gamma_m[i] = gamma_tilde_m[i] .* lambda_m[i] * scale_factor * tau_m;
  }
}
model {
  int starti = 1;
  ...
  for (i in 2:n_levels) {
    starti = starti + Z[i-1];
    endi   = starti + Z[i] - 1;
    mu_wBias[starti:endi] = mu[starti:endi] .* exp(0.15*B_m[starti:endi,:] * gamma_m[i-1] );
  }
...
generated quantities{
  ...
  for (i in 1:(n_levels-1)) {
    mu_wBias[i] = exp(0.15*B_m * gamma_m[i] );
  }
